...
async def create_seed_users(force: bool = False, production_mode: bool = False):
    ...
    if production_mode:
        ...
        if (
            ...
            os.environ["SECRET_KEY"] = (
                "temporary-secret-key-for-user-creation-script-only"
            )
    ...
    if production_mode and settings.database_connection_url.startswith("sqlite"):
        ...
        logger.warning(
            "💡 Users will be created locally, not in your production PostgreSQL"
        )
        logger.warning(
            "💡 To create users in production PostgreSQL, run this script in Render's shell"
        )
    ...
    try:
        ...
        if existing_users > 0 and not force:
            logger.info(
                f"✅ Database already has {existing_users} users. Use --force to recreate."
            )
            return True
        ...
        if production_mode:
            ...
            logger.info(
                "🚨 Creating production users with development credentials for testing"
            )
        else:
        ...
        if not production_mode:
            ...
            logger.info(
                "📚 API Backend: https://apis-for-exam-transcripts.onrender.com"
            )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force recreate users even if they exist"
    )
    parser.add_argument(
